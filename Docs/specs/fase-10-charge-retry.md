...
- Dependências: `fake-pix-provider` fase 2 (`CreateOrGet` by `payment_id`; 200 replay / 201 create). Demo/Docker: `fake-pix-provider` fase 4 (Postgres + outbox; restart does not drop charges). Tests mock `fetch`.
